...
<Update label="August 2026" description="3–28 August 2026" rss={{ title: "August 2026 — packing desk corrections, content-based import routing, refreshed interface", description: "Correct the carrier, address or quantity at the packing bench, route import files by their contents, and a refreshed interface with real record counts." }}>
  ...
  - **Security hardening.** Session tokens are destroyed on logout, closing a finding from our most recent penetration test, and an explicit organisation-level opt-out from AI model training is in place at our cloud provider. See [AI data protection](/knowledgebase/security/ai-data-protection).

  ## Fixed

  - Dates and times on goods in records and sales orders respect your subdomain's timezone rather than defaulting to server time.
  - Short receipt lines on goods in are handled correctly. See [Book goods in](/knowledgebase/inbound/book-goods-in).
  - Intermittent logouts affecting users across multiple domains are resolved. See [Problems signing in](/knowledgebase/troubleshooting/signing-in).
...
<Update label="July 2026" description="1–30 July 2026" rss={{ title: "July 2026 — self-service MFA, AI Assistant page, mixed UOM picking", description: "A security settings page with MFA in your own hands, the AI Assistant on a full page, expected dispatch times per delivery address, and mixed UOM picking." }}>
  ...
  The commercial and safety plumbing landed in the same month: AI plans can be set per tenant, which is what makes credits and entitlement work; assistant session history is retained for 180 days; and token limits are enforced per subdomain so one busy tenant cannot exhaust shared capacity.

  <Info>Start at the [AI Assistant overview](/knowledgebase/ai-assistant/overview), then [Credits and usage](/knowledgebase/ai-assistant/credits-and-usage). Your data is not used to train models — see [AI data protection](/knowledgebase/security/ai-data-protection).</Info>
...
<Update label="June 2026" description="2–29 June 2026" rss={{ title: "June 2026 — Northern Ireland shipments, recurring charges, pickfaces for everyone", description: "UKIMS references for Windsor Framework shipments, scheduled recurring charges on charge cards, and continuous pickface replenishment for all customers." }}>
  ...
  - **In-app help and feedback.** The help widget is delivered through a dedicated SDK, which makes it more reliable and lets you raise feedback without leaving Clarus.
