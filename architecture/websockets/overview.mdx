...
ActionCable applies its own allowed-origin check: each environment restricts connections to that environment's own domains, rejecting handshakes from any other origin.

Broadcasts are fanned out across API tasks through a **Redis** ActionCable adapter, so a message published on one ECS task reaches subscribers connected to any other.
