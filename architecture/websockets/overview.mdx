...
<Steps>
  ...
  <Step title="DNS resolves to CloudFront">
    Route 53 points `ws.<domain>` at the CloudFront distribution (`aws_route53_record.websocket_cf`).
  </Step>
...
| Route 53 record for `ws.<domain>` (`aws_route53_record.websocket_cf`) | `api/websocket.tf` |
