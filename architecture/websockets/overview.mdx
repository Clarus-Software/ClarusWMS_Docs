...
<Steps>
  <Step title="Client opens the socket">
    The browser connects to `wss://ws.<domain>/cable?token=<jwt>`, passing the user's Cognito JWT as a `token` query-string parameter. This hostname is a CloudFront distribution created specifically for WebSocket traffic.
  </Step>
  <Step title="DNS resolves to CloudFront">
    Route 53 points `ws.<domain>` at the CloudFront distribution.
  </Step>
...
sequenceDiagram
    ...
    C->>R53: resolve ws.<domain>
    R53-->>C: CloudFront distribution
    C->>CF: wss://ws.<domain>/cable?token=<jwt>
...
ActionCable applies its own allowed-origin check, configured per environment via `config.action_cable.allowed_request_origins`. Each environment restricts connections to that environment's own domains; the exact patterns live in the environment configs (`config/environments/*.rb`) rather than being duplicated here.
...
The edge and network components — the CloudFront distribution and its DNS alias, the Lambda@Edge authorizer, the CloudFront shared secret, the wildcard DNS that resolves the origin to the ALB, the WAF identity-header rules, and the `/cable` listener rule that routes to the WebSocket target group — are provisioned as infrastructure-as-code in the API infrastructure repository. Refer to that repository for the current resource definitions.
